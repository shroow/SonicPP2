...
void CGameObject::MoveDir(const float Speed, const Vector2& Dir)
{
	m_Pos += Dir * Speed * DELTA_TIME * m_TimeScale;
}
...
void CGameObject::MoveAngle()
{	
	m_Pos.x += cosf(DegreeToRadian(m_Angle)) * m_MoveSpeed * DELTA_TIME;
	m_Pos.y += sinf(DegreeToRadian(m_Angle)) * m_MoveSpeed * DELTA_TIME;
}

void CGameObject::MoveAngle(float Dir)
{
	m_Pos.x += cosf(DegreeToRadian(m_Angle)) * m_MoveSpeed * Dir * DELTA_TIME;
	m_Pos.y += sinf(DegreeToRadian(m_Angle)) * m_MoveSpeed * Dir * DELTA_TIME;
}

void CGameObject::MoveAngle(float Angle, float Speed)
{
	m_Pos.x += cosf(DegreeToRadian(Angle)) * Speed * DELTA_TIME;
	m_Pos.y += sinf(DegreeToRadian(Angle)) * Speed * DELTA_TIME;
}
...
void CGameObject::PostUpdate(float DeltaTime)
{
	...
	else if (m_PhysicsSimulate && m_Move.y < 0.f)
	{
		CTileMap* TileMap = m_Scene->GetTileMap();
		...
		float ResultLeft = PrevLeft < CurLeft ? PrevLeft : CurLeft;
		float ResultRight = PrevRight > CurRight ? PrevRight : CurRight;
		float ResultTop = PrevTop > CurTop ? CurTop : PrevTop;
		float ResultBottom = PrevTop < CurTop ? CurTop : PrevTop;
			...
			RightIndex >= 0 && BottomIndex >= 0)
		{
			...
			for (int i = TopIndex; i <= BottomIndex; ++i)
			{
				for (int j = LeftIndex; j <= RightIndex; ++j)
				{
					...
					float TileY = Tile->GetPos().y + Tile->GetSize().y;

					if (TileY > PrevTop)
						continue;

					// 현재 타일이 SideCollision 타일일 경우 벽에 부딪히는 것이다.
					if (Tile->GetSideCollision())
					{
						Check = true;

						// 현재 타일의 Bottom을 이 오브젝트의 Top값으로 지정한다.
						m_Pos.y = TileY + (1.f - m_Pivot.y) * m_Size.y;
...
bool CGameObject::InflictDamage(bool Damage)
{
	return Damage;
}

void CGameObject::InflictSpeed(float Speed, Vector2 Dir)
{
	m_FallTime = 0.f;
	m_Pos += Dir * Speed * m_TimeScale;
}
