...
protected:
	...
	bool		m_Start;
	bool		m_Fall;
	int			m_HitCount; //선 충돌체 충돌 카운팅
	bool		m_IsAngle;
	float		m_Angle;

	bool		m_Push; //벽에 부딪히고 추가적으로 이동을 했는지
	bool		m_Bounce;
...
public:
	...
	void MoveDir(const float Speed, const Vector2& Dir);
	void Move(const Vector2& MoveValue);
	void Move(float Angle);
	void MoveAngle();
	void MoveAngle(float Dir);
...
public:
	...
	Vector2 m_Force;
	bool	m_SetMoveRound;
	float	m_MoveRoundDir;
	Vector2 m_MoveRoundPos;
	float	m_MoveRoundAngle;

	//CircleMap
	bool GetMoveRound()	const
	{
		return m_SetMoveRound;
	}

	void SetMoveRound(bool TF, Vector2 Pos)
	{
		Pos.y += 5.f;
		m_SetMoveRound = TF;
		m_MoveRoundPos = Pos;
	...
	void SetMoveRoundAngle(float Angle, float Dir)
	{
		m_MoveRoundAngle = Angle;
		m_MoveRoundDir = Dir;
	}
