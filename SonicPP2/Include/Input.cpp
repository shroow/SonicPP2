...
bool CInput::SetMouseOn(bool OnOff)
{
	if (OnOff)
	{
		if (m_MouseOn)
		{
			if (!m_MouseObj)
			{
				...
				for (int i = 0; i <= 10; ++i)
				{
					...
					wsprintf(FileName, TEXT("UI/Mouse/%d.bmp"), i);
...
bool CInput::Init(HWND hWnd)
{
	...
	AddBindKey("Jumpkey", VK_SPACE);
	AddBindKey("Jumpkey1", 'A');
	AddBindKey("Jumpkey2", 'S');
	AddBindKey("Jumpkey3", 'D');
	AddBindKey("EnterKey", VK_RETURN);
	AddBindKey("ESCKey", VK_ESCAPE);
	
	AddBindKey("MoveUp", VK_UP);
	AddBindKey("MoveDown", VK_DOWN);
	AddBindKey("MoveRight", VK_RIGHT);
	AddBindKey("MoveLeft", VK_LEFT);
	AddBindKey("MouseLClick", VK_LBUTTON);
	AddBindKey("Skill1", '1');
	AddBindKey("OpenTileMapEditor", 'T');
	SetKeyCtrl("OpenTileMapEditor");

	AddBindKey("Fire", VK_LBUTTON);
	AddBindKey("Delete", VK_RBUTTON);
	AddBindKey("Cam1", VK_F1);
	AddBindKey("DEBUG", VK_F2);
