	...
	m_SelectTextureName{}
{
	...
	m_SelectCollider = ECollider_Type::None;
	m_ColliderStatus = -1.f;
}
...
bool CEditDlg::Init()
{
	...
	m_hDlg = CreateDialog(CGameManager::GetInst()->GetWindowInstance(),
		MAKEINTRESOURCE(IDD_DIALOG_STAGE_EDIT), CGameManager::GetInst()->GetWindowHandle(),
		CEditDlg::WndProc);

	ShowWindow(m_hDlg, SW_SHOW);

	
	//SetDlgItemInt(m_hDlg, IDC_EDIT_COUNTX, 78, TRUE);
	//SetDlgItemInt(m_hDlg, IDC_EDIT_COUNTY, 32, TRUE);
	//SetDlgItemInt(m_hDlg, IDC_EDIT_SIZEX, 96, TRUE);
	//SetDlgItemInt(m_hDlg, IDC_EDIT_SIZEY, 48, TRUE);	
	

	SetDlgItemInt(m_hDlg, IDC_EDIT_COUNTX, 258, TRUE);
	SetDlgItemInt(m_hDlg, IDC_EDIT_COUNTY, 96, TRUE);
	SetDlgItemInt(m_hDlg, IDC_EDIT_SIZEX, 32, TRUE);
	SetDlgItemInt(m_hDlg, IDC_EDIT_SIZEY, 16, TRUE);
	...
	m_hColliderRadius = GetDlgItem(m_hDlg, IDC_CHECK_COLLIDER_ON);
	m_hDeleteRadius = GetDlgItem(m_hDlg, IDC_CHECK_DELETE_COLLIDER);
	m_hColliderCombo = GetDlgItem(m_hDlg, IDC_COMBO_COLLIDER_TYPE);
	...
	SendMessage(m_hSideCollisionCheckBox, BM_SETCHECK, (WPARAM)BST_CHECKED, 0);

	//SendMessage(m_hColliderRadius, BM_SETCHECK, (WPARAM)BST_CHECKED, 0);
	//SendMessage(m_hDeleteRadius, BM_SETCHECK, (WPARAM)BST_CHECKED, 0);

	SendMessage(m_hColliderCombo, CB_ADDSTRING, 0, (LPARAM)TEXT("None"));
	SendMessage(m_hColliderCombo, CB_ADDSTRING, 0, (LPARAM)TEXT("Rectangle"));
	SendMessage(m_hColliderCombo, CB_ADDSTRING, 0, (LPARAM)TEXT("Line"));
	SendMessage(m_hColliderCombo, CB_ADDSTRING, 0, (LPARAM)TEXT("Circle"));
	SendMessage(m_hColliderCombo, CB_SETCURSEL, 0, 0);
...
{
	m_SelectCollider = (ECollider_Type)SendMessage(m_hColliderCombo, CB_GETCURSEL, 0, 0);
}

float CEditDlg::GetLineBuffer() const
{
	if (!m_hColliderRadius)
		return -1.f;

	if (m_hDeleteRadius)
		return -1.f;

	if (m_SelectCollider != ECollider_Type::Line)
		return -1.f;
	
	if (m_ColliderStatus == -1.f)
		return -1.f;
	...
	Buffer = (float)GetDlgItemInt(m_hDlg, IDC_EDIT_LINE_BUFFER, &Transfer, TRUE);
